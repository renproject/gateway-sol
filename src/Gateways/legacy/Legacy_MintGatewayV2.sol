...
contract Legacy_MintGatewayStateV2 is Legacy_MintGatewayStateV1 {
    struct Burn {
...
contract Legacy_MintGatewayLogicV2 is
    Initializable,
    Legacy_Claimable,
    ...
    Legacy_MintGatewayStateV2
{
    ...
    function transferTokenOwnership(Legacy_MintGatewayLogicV2 _nextTokenOwner) public onlyOwner {
    ...
    function updateMintAuthority(address _nextMintAuthority) public onlyOwnerOrMintAuthority {
        ...
        require(_nextMintAuthority != address(0), "MintGateway: mintAuthority cannot be set to address zero");
    ...
    function updateFeeRecipient(address _nextFeeRecipient) public onlyOwner {
        ...
        require(_nextFeeRecipient != address(0x0), "MintGateway: fee recipient cannot be 0x0");
    ...
    /// @param _nextMintFee The new fee for minting and burning.
    /// @param _nextBurnFee The new fee for minting and burning.
    function updateFees(uint16 _nextMintFee, uint16 _nextBurnFee) public onlyOwner {
        mintFee = _nextMintFee;
        burnFee = _nextBurnFee;
    }
    ...
    function mint(
        ...
        bytes32 sigHash = hashForSignature(_pHash, _amountUnderlying, msg.sender, _nHash);

        // Calculate the v0.2 signature hash for backwards-compatibility.
        bytes32 legacySigHash = _legacy_hashForSignature(_pHash, _amountUnderlying, msg.sender, _nHash);

        // Check that neither signature has been redeemed.
        require(status[sigHash] == false && status[legacySigHash] == false, "MintGateway: nonce hash already spent");

        // If both signatures fail verification, throw an error. If any one of
        // them passed the verification, continue.
        if (!verifySignature(sigHash, _sig) && !verifySignature(legacySigHash, _sig)) {
        ...
        uint256 receivedAmountScaled = amountScaled.sub(absoluteFeeScaled, "MintGateway: fee exceeds amount");
    ...
    function burn(bytes memory _to, uint256 _amount) public returns (uint256) {
        ...
        uint256 amountAfterFee = _amount.sub(fee, "MintGateway: fee exceeds amount");
        ...
        Legacy_MintGatewayStateV2.burns[nextN] = Burn({
    ...
    {
        Burn memory burnStruct = Legacy_MintGatewayStateV2.burns[_n];
    ...
    function hashForSignature(
        ...
        return keccak256(abi.encode(_pHash, _amount, selectorHash, _to, _nHash));
    }
...
contract Legacy_MintGatewayProxy is InitializableAdminUpgradeabilityProxy {
