...
const setup = deployments.createFixture(async () => {
    ...
    await deployments.fixture("RenVMSignatureVerifierV1");
    await deployments.fixture("RenVMSignatureVerifierProxy");
    ...
    const contracts = {
        ...
        BasicBridge: <BasicBridge>(<unknown>await ethers.getContract("BasicBridge")),
        RenVMSignatureVerifier: <RenVMSignatureVerifierV1>(
            (<unknown>(
                await ethers.getContractAt(
                    "RenVMSignatureVerifierV1",
                    (
                        await ethers.getContract("RenVMSignatureVerifierProxy")
                    ).address
                )
            ))
        ),
    ...
    const deployer = (await setupUsers([(await getNamedAccounts()).deployer], contracts))[0];
    ...
    await deployer.RenVMSignatureVerifier.updateMintAuthority(mintAuthority);

    // Set up Ethereum network config.
    const providerNetwork = await ethers.provider.getNetwork();
    const networkID = providerNetwork ? providerNetwork.chainId : 0;
    const network = LocalEthereumNetwork(networkID, contracts.GatewayRegistryV2.address, contracts.BasicBridge.address);

    const signer = await ethers.getSigner(user.address);
    const ethereum = Ethereum({ provider: signer.provider!, signer: await ethers.getSigner(user.address) }, network);
...
describe("RenJS", () => {
    it("mint and burn", async function () {
        ...
        const renBTC = await (<RenAssetV2>(<unknown>await ethers.getContractAt("RenAssetV2", renBTCAddress))).connect(
            await ethers.getSigner(users[0].address)
        );

        const amountToBurn = await renBTC.balanceOf(users[0].address);

        await (await renBTC.approve(users[0].BasicBridge.address, amountToBurn)).wait();
